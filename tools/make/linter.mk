...
shellcheck: ## Lint all shell scripts in the project
	...
	@shellcheck -e SC2155,SC2034,SC1091,SC2011,SC2012,SC2087,SC2119,SC2120,SC2162 $(shell find . -type f -name "*.sh" -not -path "./node_modules/*" -not -path "./website/node_modules/*" -not -path "./dashboard/frontend/node_modules/*" -not -path "./models/*" -not -path "./.augment/*" -not -path "./.venv/*" -not -path "*/.venv/*" -not -path "./.venv-agent/*" -not -path "./.venv-codex/*" -not -path "./.codex-agent-venv/*")
