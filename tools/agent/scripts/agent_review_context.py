...
def historical_briefs(
    *,
    ...
    review_root = repo_root / "docs" / "agent" / "reviews"
    if not review_root.is_dir() or max_count <= 0:
        return []

    module_tokens = {module.lower().replace("/", "-") for module in modules}
    candidates: list[Path] = []
    for path in sorted(review_root.glob("*/*.md"), reverse=True):
    ...
    for path in candidates[:max_count]:
        relative = path.relative_to(repo_root).as_posix()
        text = read_existing_file(path, DEFAULT_MAX_BRIEF_BYTES)
        if text is not None:
...
def build_review_prompt(inputs: ContextInputs, *, max_historical_briefs: int) -> str:
...
def main() -> int:
    ...
    parser.add_argument(
        "--max-historical-briefs",
        type=int,
        default=DEFAULT_MAX_HISTORICAL_BRIEFS,
    )
    ...
    prompt = build_review_prompt(
        inputs,
        max_historical_briefs=args.max_historical_briefs,
    )
